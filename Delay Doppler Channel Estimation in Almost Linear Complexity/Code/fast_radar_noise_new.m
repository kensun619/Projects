function [error, SNR_dB_rec_L, SNR_dB_rec_M, tau_dec, omega_dec] = fast_radar_noise_new(tau,omega,alpha,SNR_dB,N,param)
...
%check mismatch
if( num_targets ~= num_dec_targets_L || num_targets ~= num_dec_targets_M )
   error = 1;
   tau_dec = [];
   omega_dec = [];
   if displayAm
       disp('number of detection mismatch');
   end
   return
end
...
tau_tmp = tau_candidate(1:num_dec_targets_L);
omega_tmp = omega_candidate(1:num_dec_targets_M);
